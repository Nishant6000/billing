...
const messageFor = (template, contact, settings) => template
  .replaceAll('{name}', contact.name || t('customer'))
...
const uniqueContacts = (rows) => {
  ...
  rows.forEach(row => {
    ...
    map.set(phone, {
      name: row.name || existing?.name || t('customer'),
      phone,
...
const renderContactRows = (search = '') => {
  ...
  $('#marketing-count').textContent = `${selectedPhones.size} ${t('selected')} / ${contacts.length} ${t('customers').toLowerCase()}`;
  $('#marketing-body').innerHTML = filtered.length ? filtered.map(contact => `
    <tr class="touch-row">
      ...
      <td><strong>${escapeHtml(contact.name)}</strong><div class="text-muted small">${escapeHtml(contact.source || t('imported'))}</div></td>
  ...
  `).join('') : `<tr><td colspan="4" class="text-center text-muted py-4">${t('noMarketingCustomers')}</td></tr>`;
};

export const renderWhatsappMarketing = async () => {
  await loadCustomers();
  const savedTemplate = localStorage.getItem('pos-whatsapp-marketing-template') || t('defaultMarketingTemplate');
  $('#view').innerHTML = `
    <div class="row g-3">
      <div class="col-xl-5">
        <div class="pos-card">
          <h2 class="section-title">${t('marketingMessage')}</h2>
          <label class="form-label">${t('message')}</label>
          <textarea class="form-control" id="marketing-message" rows="8" placeholder="${t('writeMarketingMessage')}">${escapeHtml(savedTemplate)}</textarea>
          <div class="text-muted small mt-2">${t('usePlaceholders')}: <code>{name}</code>, <code>{phone}</code>, <code>{shop}</code></div>
          <div class="row g-2 mt-3">
            <div class="col-md-6">
              <label class="form-label">${t('uploadMessageTxt')}</label>
              <input class="form-control" id="message-upload" type="file" accept=".txt,text/plain">
            </div>
            <div class="col-md-6">
              <label class="form-label">${t('importContactsCsv')}</label>
          ...
          <div class="form-check mt-3">
            ...
            <label class="form-check-label" for="marketing-consent">${t('whatsappConsent')}</label>
          </div>
          <div class="d-grid gap-2 mt-3">
            <button class="btn btn-primary-gradient" id="open-next-whatsapp"><i class="fa-brands fa-whatsapp"></i> ${t('openNextSelectedChat')}</button>
            <button class="btn btn-outline-success" id="open-all-whatsapp"><i class="fa-solid fa-list-check"></i> ${t('prepareAllLinks')}</button>
            <button class="btn btn-outline-secondary" id="save-marketing-template"><i class="fa-solid fa-floppy-disk"></i> ${t('saveTemplate')}</button>
          </div>
        </div>
      </div>
      <div class="col-xl-7">
        <div class="pos-card">
          <div class="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-3">
            <div><h2 class="section-title mb-1">${t('customers')}</h2><p class="text-muted mb-0" id="marketing-count"></p></div>
            <div class="d-flex gap-2 flex-wrap">
              <button class="btn btn-outline-primary" id="select-all-customers"><i class="fa-solid fa-check-double"></i> ${t('selectAll')}</button>
              <button class="btn btn-outline-secondary" id="clear-customer-selection"><i class="fa-solid fa-xmark"></i> ${t('clear')}</button>
              <button class="btn btn-outline-success" id="export-customers"><i class="fa-solid fa-download"></i> ${t('export')}</button>
            </div>
          </div>
          <input class="form-control mb-3" id="marketing-search" placeholder="${t('searchCustomerMobile')}">
          <div class="table-responsive">
            <table class="table">
              <thead><tr><th></th><th>${t('customer')}</th><th>${t('mobile')}</th><th></th></tr></thead>
...
const canSend = () => {
  if (!$('#marketing-consent').checked) {
    toast(t('confirmWhatsappConsent'), 'warning');
    return false;
  }
  if (!$('#marketing-message').value.trim()) {
    toast(t('enterMarketingMessage'), 'warning');
    return false;
  }
  return true;
};
...
const openNextSelected = async () => {
  ...
  if (!selected.length) return toast(t('selectAtLeastOneCustomer'), 'warning');
  const contact = selected[currentIndex % selected.length];
  currentIndex += 1;
  await openContact(contact.phone);
  toast(`${t('opened')} ${contact.name}`);
};
...
const loadContactsFile = async (event) => {
  ...
  const imported = text.split(/\r?\n/)
    ...
    .map(([name, phone]) => ({ name, phone, source: t('csvImport') }));
  contacts = uniqueContacts([...contacts, ...imported]);
  selectedPhones = new Set(contacts.map(contact => contact.phone));
  renderContactRows($('#marketing-search').value);
  toast(t('contactsImported'));
};
