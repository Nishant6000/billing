    ...
    <div class="pos-card">
      <form id="purchase-form">
        <div class="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-3">
          <div>
            <h2 class="section-title mb-1">Purchase Bill Entry</h2>
            <p class="text-muted mb-0">Enter supplier purchase bills for input GST tracking.</p>
          </div>
          <a class="btn btn-outline-primary" href="#/purchase-history"><i class="fa-solid fa-clock-rotate-left"></i> Purchase History</a>
        </div>
          <div class="row g-3">
            <div class="col-md-6"><label class="form-label">Purchase Bill No</label><input class="form-control" name="purchase_no" required></div>
            <div class="col-md-6"><label class="form-label">Bill Date</label><input class="form-control" name="bill_date" type="date" value="${dateOnly()}" required></div>
            <div class="col-md-6"><label class="form-label">Supplier Name</label><input class="form-control" name="supplier_name" required></div>
            <div class="col-md-6"><label class="form-label">Supplier GSTIN</label><input class="form-control" name="supplier_gstin"></div>
          ...
          <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <button class="btn btn-sm btn-outline-primary" type="button" id="add-purchase-item"><i class="fa-solid fa-plus"></i> Add Item</button>
          </div>
          <div id="purchase-items"></div>
          <div class="mt-3" id="purchase-totals"></div>
          <button class="btn btn-primary-gradient mt-3"><i class="fa-solid fa-floppy-disk"></i> Save Purchase</button>
      </form>
    </div>
  `;
    ...
    <div class="pos-card">
      <div class="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-3">
        <div>
          ...
          <p class="text-muted mb-0">View and manage saved supplier purchase bills.</p>
        </div>
        <div class="d-flex gap-2 flex-wrap">
          ...
          <input class="form-control" id="purchase-search" style="max-width: 280px" placeholder="Search bill, supplier, GSTIN">
          ...
          <a class="btn btn-primary-gradient" href="#/purchase"><i class="fa-solid fa-plus"></i> New Purchase</a>
        </div>
      </div>
      <div class="table-responsive">
        <table class="table">
          <thead><tr><th>Bill</th><th>Supplier</th><th>Date</th><th>GST</th><th>Total</th><th></th></tr></thead>
...
const renderPurchaseItems = () => {
  $('#purchase-items').innerHTML = purchaseItems.map((item, index) => `
    <div class="purchase-item-row">
      <div><label class="form-label">Item</label><input class="form-control" data-purchase-field="item_name" data-index="${index}" value="${escapeHtml(item.item_name)}" required></div>
      <div><label class="form-label">HSN</label><input class="form-control" data-purchase-field="hsn" data-index="${index}" value="${escapeHtml(item.hsn)}"></div>
      <div><label class="form-label">Qty</label><input class="form-control" type="number" min="0.01" step="0.01" data-purchase-field="quantity" data-index="${index}" value="${item.quantity}"></div>
      <div><label class="form-label">Taxable</label><input class="form-control" type="number" min="0" step="0.01" data-purchase-field="taxable_value" data-index="${index}" value="${item.taxable_value}"></div>
...
const renderPurchaseTotals = () => {
  ...
  $('#purchase-totals').innerHTML = `
    <div class="d-flex justify-content-between"><span>Taxable Value</span><strong>${money(totals.subtotal)}</strong></div>
    <div class="d-flex justify-content-between"><span>Input GST</span><strong>${money(totals.gst)}</strong></div>
    <hr><div class="d-flex justify-content-between fs-5"><span>Total</span><strong>${money(totals.total)}</strong></div>
  `;
};
...
const renderPurchaseRows = async () => {
  ...
  `).join('') : '<tr><td colspan="6" class="text-center text-muted py-4">No purchase bills found.</td></tr>';
};
